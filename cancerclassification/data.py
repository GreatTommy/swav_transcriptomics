...
def read_process_data_TCGA(
    path="/home/commun/data/RNASeq/TCGA/release_2021/processed/PanCancer/ALL/",
):
    """Reads and processes (including a normal standardardization) the TCGA data.

    Args:
        path (str, optional): Path of the folder containing TCGA's csv files. Defaults
            to "/home/commun/data/RNASeq/TCGA/release_2021/processed/PanCancer/ALL/".

    Returns:
        numpy.ndarray: Numpy array of the processed data.
    """
    class_df = pd.read_csv(os.path.join(path, "label.csv"))
    data_df = pd.read_csv(os.path.join(path, "RNA-Seq.omics.csv"))
    ...
    df = df.drop(columns=list(df.columns[:7]) + [df.columns[8]])  # columns management
...
def read_process_data_MA(
    path="/home/commun/data/MicroArray/E-MTAB-3732/", selected_type=None
):
    ...
    data_df = pd.read_csv(os.path.join(path, "E-MTAB-3732.data2.csv"))
...
def get_dataloaders(dataset, idx, bs=None, fast=True, verbose=True):
...
class LogResults:
    ...
    def __init__(self, name, hyp_str, hyp_vals=None):
        ...
        self.df_results = pd.DataFrame(
            columns=["id", "epoch", "val_acc", "val_loss", "test_acc", "test_loss"]
            + hyp_str
        )
        self.hyp_vals = hyp_vals

    def log_epoch(
        self, epoch=None, valacc=None, valloss=None, testacc=None, testloss=None
    ):
        ...
        """
        new_serie = pd.Series(
            [f"run-{self.counter}", epoch, valacc, valloss, testacc, testloss]
            + self.hyp_vals,
